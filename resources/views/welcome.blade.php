...
<style>
    ...
    <script src="https://kit.fontawesome.com/918ca60ec1.js" crossorigin="anonymous"></script>
    *,
    *::before,
    *::after {
        margin: 0;
        ...
        <section class="sections benefits">
            <div class="benefit-center box">
                ...
                <div class="benefit">
                    ...
                    <span class="text">Only Original Products</span>
                </div>
            ...
            <div class="ac-center box">
                ...
                <div class="ac">
                    ...
                    @if($latest['sale_price']>0)
                        <div class="price">
                            <strike style="color: #761b18;">{{$latest['price']}} DH</strike>
                            <span style="color: #2d995b">{{$latest['sale_price']}} DH</span>
                        </div>
                        <a href="{{route('products.productshop')}}" class="btn btn-outline-success">ORDER NOW</a>
                    @else
                        <div class="price">{{$latest['price']}} DH</div>
                        <a href="{{route('products.productshop')}}" class="btn btn-outline-success">ORDER NOW</a>
                    @endif
                </div>
                @endforeach
        ...
        <section class="sections">
            ...
            <div class="ab box">
                <div class="item item-1">
                    @if(!empty($categories) && isset($categories->toArray()[0]['image_cat']) && is_file('uploads/categories/'.$categories->toArray()[0]['image_cat']))
                        <img src="{{ asset('/uploads/categories/'.$categories->toArray()[0]['image_cat']) }}" alt="">
                    @elseif(!empty($categories) && isset($categories->toArray()[0]['image_cat']))
                        <img src="{{url($categories->toArray()[0]['image_cat'])}}" alt="">
                    @endif

                </div>
                <div class="item item-2">
                    @if(!empty($categories) && isset($categories->toArray()[1]['image_cat']) && is_file('uploads/categories/'.$categories->toArray()[1]['image_cat']))
                        <img src="{{ asset('/uploads/categories/'.$categories->toArray()[1]['image_cat']) }}" alt="">
                    @elseif(!empty($categories) && isset($categories->toArray()[1]['image_cat']))
                        <img src="{{url($categories->toArray()[1]['image_cat'])}}" alt="">
                    @endif
                </div>
                <div class="item item-3">
                    @if(!empty($categories) && isset($categories->toArray()[2]['image_cat']) && is_file('uploads/categories/'.$categories->toArray()[2]['image_cat']))
                        <img src="{{ asset('/uploads/categories/'.$categories->toArray()[2]['image_cat']) }}" alt="">
                    @elseif(!empty($categories) && isset($categories->toArray()[3]['image_cat']))
                        <img src="{{url($categories->toArray()[3]['image_cat'])}}" alt="">
                    @endif
                </div>
                <div class="item item-4">
                    @if(isset($categories[3]) && is_file('uploads/categories/'.$categories[3]['image_cat']))
                        <img src="{{ asset('/uploads/categories/'.$categories[3]['image_cat']) }}" alt="">
                    @elseif(isset($categories[3]))
                        <img src="{{url($categories[3]['image_cat'])}}" alt="">
                    @else
                        <!-- Handle the case where the fourth category does not exist -->
                        {{-- <img src="{{url('default-image.jpg')}}" alt=""> --}}
                    @endif
                </div>
            </div>
        </section>
            ...
            <div class="ac-center box">
                ...
                <div class="ac">
                    ...
                    @if($product['sale_price']>0)
                        <div class="price">
                            <strike style="color: #761b18;">{{$product['price']}} DH</strike>
                            <span style="color: #2d995b">{{$product['sale_price']}} DH</span>
                        </div>
                        <a href="{{route('products.productshop')}}" class="btn btn-outline-success">ORDER NOW</a>
                    @else
                        <div class="price">DH {{$product['price']}}</div>
                        <a href="{{route('products.productshop')}}" class="btn btn-outline-success">ORDER NOW</a>
                    @endif
{{--                    <div class="price">8500 DH</div>--}}
                </div>
                @endforeach
            ...
            <div class="blog-post">
                <h2 class="blog-post-title" style="text-align: center;">User Management Section</h2> 
                <p>
